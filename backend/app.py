...
def get_all_restaurants():
    ...
    if minRating is None:
        return create_response(status=404, message='Error: invalid minRating')

    filtered_restaurants = []

    for restaurant in restaurants:
      if restaurant.get('rating') >= int(minRating):
        filtered_restaurants.append(restaurant)
    if len(filtered_restaurants) == 0:
      return create_response(status=404, message='Error: no restaurants found with given minRating')
    return create_response({"restaurants": filtered_restaurants})

@app.route("/restaurants/<id>", methods=['GET'])
def get_specified_restaurant(id):
    restaurant = db.getById('restaurants', int(id))
    if restaurant is None:
      return create_response(status=404, message='Error: no restaurant with id found')
